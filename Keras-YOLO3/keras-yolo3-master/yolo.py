...
class YOLO(object):
    ...
    def generate(self):
        model_path = os.path.expanduser(self.model_path)  # 载入模型参数文件
        ...
        try:
            self.yolo_model = load_model(model_path, compile=False)  # 入模型参数文件
        except:
    ...
    def detect_image(self, image):
        '''
        并返回边框boxes,置信度score和类别名称
        :param image:
        :return:
        '''
        start = timer()

        if self.model_image_size != (None, None):  # 确定定义的输入图片尺寸是32的倍数
            assert self.model_image_size[0]%32 == 0, 'Multiples of 32 required'
            assert self.model_image_size[1]%32 == 0, 'Multiples of 32 required'
            # 改变图片尺寸为[416,416]  这里不改变图片比例,较小的用灰色填充
        ...
        out_boxes, out_scores, out_classes = self.sess.run(
            ...
            feed_dict={
                self.yolo_model.input: image_data,  # 传入图数据、图片尺寸等参数
...
def detect_video(yolo, video_path, output_path=""):
    ...
    if isOutput:
        ...
        out = cv2.VideoWriter(output_path, video_FourCC, video_fps, video_size)  #创建写入对象
    accum_time = 0
    curr_fps = 0
    fps = "FPS: ??"
    prev_time = timer()
    while True:
        return_value, frame = vid.read()  # 读取视频
        image = Image.fromarray(frame)  # OpenCV转换成PIL.Image格式
        image = yolo.detect_image(image)  # 开始对每帧进行检测
